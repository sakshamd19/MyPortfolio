...
function renderGraph() {
    ...
    const voltosActivityLogs = {};
    voltosProgressRecords.forEach(record => {
        if (voltosActivityLogs[record.date]) {
            voltosActivityLogs[record.date].level = Math.min(4, voltosActivityLogs[record.date].level + 1);
            voltosActivityLogs[record.date].log += " | " + record.title;
        } else {
            voltosActivityLogs[record.date] = { level: 2, log: record.title }; 
        }
    });
    ...
    for (let i = 0; i < totalCells; i++) {
        ...
        if (voltosActivityLogs[dateStr]) {
            const level = voltosActivityLogs[dateStr].level;
            cell.classList.add(`level-${level}`);
            cell.title = `${dateStr}: ${voltosActivityLogs[dateStr].log}`;
        }
...
function renderTimeline() {
    const container = document.getElementById('voltos-timeline-container');
    if (!container) return;
    
    container.innerHTML = ''; // Clear existing
    
    // Sort records by date descending (newest first)
    const sortedRecords = [...voltosProgressRecords].sort((a, b) => new Date(b.date) - new Date(a.date));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        form.onsubmit = (e) => {
            ...
            voltosProgressRecords.push({ date, title, description: desc });
            localStorage.setItem('voltosProgress', JSON.stringify(voltosProgressRecords));
