...
function renderGraph() {
    ...
    const evalithmActivityLogs = {};
    evalithmProgressRecords.forEach(record => {
        if (evalithmActivityLogs[record.date]) {
            evalithmActivityLogs[record.date].level = Math.min(4, evalithmActivityLogs[record.date].level + 1);
            evalithmActivityLogs[record.date].log += " | " + record.title;
        } else {
            evalithmActivityLogs[record.date] = { level: 2, log: record.title }; 
        }
    });
    ...
    for (let i = 0; i < totalCells; i++) {
        ...
        if (evalithmActivityLogs[dateStr]) {
            const level = evalithmActivityLogs[dateStr].level;
            cell.classList.add(`level-${level}`);
            cell.title = `${dateStr}: ${evalithmActivityLogs[dateStr].log}`;
        }
...
function renderTimeline() {
    const container = document.getElementById('evalithm-timeline-container');
    if (!container) return;
    
    container.innerHTML = ''; // Clear existing
    
    // Sort records by date descending (newest first)
    const sortedRecords = [...evalithmProgressRecords].sort((a, b) => new Date(b.date) - new Date(a.date));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        form.onsubmit = (e) => {
            ...
            evalithmProgressRecords.push({ date, title, description: desc });
            localStorage.setItem('evalithmProgress', JSON.stringify(evalithmProgressRecords));
